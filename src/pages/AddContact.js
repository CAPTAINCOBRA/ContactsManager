...
const AddContact = () => {
  ...
  const imagePicker = async (e) => {
    ...
    try {
      ...
      var uploadTask = storageRef
        .child("images/" + file.name + Date.now()) //Made a custom change here DONE:
        .put(resizedImage, metadata);

      uploadTask.on(
        firebase.storage.TaskEvent.STATE_CHANGED, //This snapshot is returned in the callback and that is how we are grabbing it
        (snapshot) => {
          setIsUploading(true);
          var progress =
        ...
        }, //Another callback available here to do whatever we want
        () => {
  ...
  const addContact = async () => {
    //TODO: DONE:l add contact method
    try {
      firebase
        .database()
        .ref("contacts/" + v4()) //creating a subsection named contacts/uniqueId in the realtime db
        .set({
          name,
          email,
          phoneNumber,
  ...
  const handleSubmit = (e) => {
    ...
    //The below part is just to handle the bug
    dispatch({
